...
import { listPatients, getPatient, runSequence } from '../api/client';
import {
...
export default function InterventionSequencer() {
    const { data: patientsResp, loading: patientsLoading } = useApi(listPatients, { key: 'seq-patients' });
            ...
            {!selectedId && !patientsLoading && (
                <div className="glass-card" style={{ textAlign: 'center', padding: 'var(--sp-8)' }}>
                    ...
                    <p style={{ fontSize: '1.1rem', color: 'var(--text-secondary)', marginBottom: 'var(--sp-2)' }}>
                        Build a multi-step treatment plan
                    </p>
                    <p className="data-label">
                        Select a patient, then arrange interventions in sequence to simulate combined outcomes
                    </p>
                </div>
            )}
        </motion.div>
