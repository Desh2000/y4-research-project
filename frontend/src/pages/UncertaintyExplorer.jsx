...
import { listPatients, getPatient, evaluateUncertainty } from '../api/client';
import {
...
export default function UncertaintyExplorer() {
    const { data: patientsResp, loading: patientsLoading } = useApi(listPatients, { key: 'unc-patients' });
            ...
            {!selectedId && !patientsLoading && !loading && (
                <div className="glass-card" style={{ textAlign: 'center', padding: 'var(--sp-8)' }}>
                    ...
                    <p style={{ fontSize: '1.1rem', color: 'var(--text-secondary)', marginBottom: 'var(--sp-2)' }}>
                        Select a patient to evaluate prediction uncertainty
                    </p>
                    <p className="data-label">
                        MC Dropout reveals how confident the LSTM really is — beyond single-pass softmax scores
                    </p>
                </div>
            )}
        </motion.div>
